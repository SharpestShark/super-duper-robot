...
function start() {
  ...
  health.value = 100;
  hpVis = new component("12px", "Consolas", "black", 12, 410, "text");
...
var x, y, width, height, gap, minWidth, maxWidth, minHeight, maxHeight, minGap, maxGap;
  ...
  myLeftBtn.update(); 
  myRightBtn.update();
  if ((health.value <= 50) && (health.value > 25)) {health.color = "#ffff00";} else if ((health.value <= 25) && (health.value >= 10)) {health.color = "#ff9900";} else if (health.value <= 10) {health.color = "red";}
  health.update();
  hpVis.text = health.value.toString();
  hpVis.update();
  player.update();
